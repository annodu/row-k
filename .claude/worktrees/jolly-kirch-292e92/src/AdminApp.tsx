...
export function AdminApp() {
  ...
  const [overviewListFilter, setOverviewListFilter] = useState<string | null>(null);
  const [freshnessFilter, setFreshnessFilter] = useState<"links" | "services" | null>(null);
  const [draftsFilter, setDraftsFilter] = useState<"needs_review" | "ready_to_approve" | null>(null);
  const [discoveryFilter, setDiscoveryFilter] = useState<"high" | "needs_review" | null>(null);
  ...
  async function loadAdminData() {
    setIsBusy(true);
    try {
      const [draftResponse, optionResponse, dashboardResponse, discoveryResponse, savedChecksResponse] = await Promise.all([
        ...
        fetch("/api/admin/freshness-saved-checks", { credentials: "include" }),
      ]);
      ...
      if (savedChecksPayload?.checks?.length) {
        setChecks(savedChecksPayload.checks);
        setChecksLoadedAt(savedChecksPayload.checkedAt ?? "");
        setCheckProgress({ checkedCount: savedChecksPayload.checkedCount ?? 0, total: savedChecksPayload.total ?? 0, nextOffset: null });
      }
    } finally {
      setIsBusy(false);
    }
    ...
    <main className="min-h-screen bg-stone-100 text-stone-950">
      ...
      {activeView === "overview" ? (
        <DashboardOverview
          ...
          listFilter={overviewListFilter}
          onListFilterChange={setOverviewListFilter}
          intakeText={intakeText}
          isBusy={isBusy}
          onIntakeChange={setIntakeText}
          onSubmit={createBulkDrafts}
          onOpenView={(view) => { setOverviewListFilter(null); setActiveView(view); }}
        />
      ) : null}
        ...
        <div className="mx-auto max-w-7xl px-5 pt-5">
          <div className="grid gap-4 lg:grid-cols-3">
            <DashboardCard
              ...
              isActive={draftsFilter === null}
              onClick={() => setDraftsFilter(null)}
            />
            <DashboardCard
              title="Needs review"
              value={dashboard?.drafts.needsReview ?? 0}
              isActive={draftsFilter === "needs_review"}
              onClick={() => setDraftsFilter(draftsFilter === "needs_review" ? null : "needs_review")}
            />
            <DashboardCard
              title="Ready to publish"
              value={dashboard?.drafts.readyToApprove ?? 0}
              isActive={draftsFilter === "ready_to_approve"}
              onClick={() => setDraftsFilter(draftsFilter === "ready_to_approve" ? null : "ready_to_approve")}
            />
          </div>
        </div>
      ) : null}
        ...
        <div className="mx-auto max-w-7xl px-5 pt-5">
          <div className="grid gap-4 lg:grid-cols-3">
            <DashboardCard
              ...
              isActive={discoveryFilter === null}
              onClick={() => setDiscoveryFilter(null)}
            />
            <DashboardCard
              title="High confidence"
              value={dashboard?.discovery.highConfidence ?? 0}
              isActive={discoveryFilter === "high"}
              onClick={() => setDiscoveryFilter(discoveryFilter === "high" ? null : "high")}
            />
            <DashboardCard
              title="To review"
              value={dashboard?.discovery.needsReview ?? 0}
              isActive={discoveryFilter === "needs_review"}
              onClick={() => setDiscoveryFilter(discoveryFilter === "needs_review" ? null : "needs_review")}
            />
          </div>
        </div>
      ) : null}
        ...
        <div className="mx-auto max-w-7xl px-5 pt-5">
          <div className="grid gap-4 lg:grid-cols-3">
            <DashboardCard
              ...
              isActive={freshnessFilter === null}
              onClick={() => setFreshnessFilter(null)}
            />
            <DashboardCard
              title="Link issues"
              value={dashboard?.freshness.brokenLinks ?? 0}
              isActive={freshnessFilter === "links"}
              onClick={() => setFreshnessFilter(freshnessFilter === "links" ? null : "links")}
            />
            <DashboardCard
              title="Service changes"
              value={dashboard?.freshness.serviceChanges ?? 0}
              isActive={freshnessFilter === "services"}
              onClick={() => setFreshnessFilter(freshnessFilter === "services" ? null : "services")}
            />
          </div>
        </div>
      ) : null}
        ...
        <section className="space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary">{drafts.filter((d) => !draftsFilter || d.status === draftsFilter).length}</Badge>
            </div>
            {drafts.filter((d) => !draftsFilter || d.status === draftsFilter).length === 0 ? (
              <p className="rounded-md border border-dashed border-stone-300 bg-white p-4 text-sm text-stone-500">No drafts waiting.</p>
            ) : (
              drafts.filter((d) => !draftsFilter || d.status === draftsFilter).map((draft) => (
                <button
                  key={draft.id}
                  type="button"
              ...
              {isRunningChecks ? (
                ...
                <>
                  <div className="mt-4 max-h-72 space-y-2 overflow-auto">
                    {checks
                      .filter((check) => {
                        if (freshnessFilter === "links") return check.linkChecks.some((l) => l.status !== "ok");
                        if (freshnessFilter === "services") return check.addedServices.length > 0 || check.removedServices.length > 0;
                        return true;
                      })
                      .map((check) => (
          ...
          {activeView === "discovery" ? (
            <DiscoveryPanel
              suggestions={suggestions.filter((s) => {
                if (discoveryFilter === "high") return s.confidence === "high";
                if (discoveryFilter === "needs_review") return s.status === "suggested";
                return true;
              })}
...
function DashboardOverview({
  ...
  function toggle(filter: string) {
    onListFilterChange(listFilter === filter ? null : filter);
  }

  const filteredDrafts =
    listFilter === "drafts-needs-review"
      ? drafts.filter((d) => d.status === "needs_review")
      : listFilter === "drafts-ready"
        ? drafts.filter((d) => d.status === "ready_to_approve")
        : listFilter === "drafts-all"
          ? drafts
          : null;

  const filteredSuggestions =
    listFilter === "discovery-high"
      ? suggestions.filter((s) => s.confidence === "high")
      : listFilter === "discovery-all"
        ? suggestions
        : null;
    ...
    <div className="mx-auto max-w-7xl space-y-5 px-5 py-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <DashboardCard
          title="Draft"
          value={dashboard?.drafts.needsReview ?? 0}
          isActive={listFilter === "drafts-needs-review"}
          onClick={() => toggle("drafts-needs-review")}
        />
        <DashboardCard
          title="Ready to publish"
          value={dashboard?.drafts.readyToApprove ?? 0}
          isActive={listFilter === "drafts-ready"}
          onClick={() => toggle("drafts-ready")}
        />
        <DashboardCard
          title="Published"
          value={dashboard?.freshness.total ?? 0}
          isActive={false}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <DashboardCard
          ...
          isActive={listFilter === "drafts-all"}
          onClick={() => toggle("drafts-all")}
        />
        <DashboardCard
          title="Stale entries"
          value={dashboard?.freshness.totalIssues ?? 0}
        ...
        <DashboardCard
          ...
          isActive={listFilter === "discovery-all"}
          onClick={() => toggle("discovery-all")}
        />
      </div>
      ...
      {filteredDrafts !== null ? (
        <div className="rounded-md border border-stone-200 bg-white">
          ...
          {filteredDrafts.length === 0 ? (
            <p className="px-4 py-5 text-sm text-stone-400">No entries in this category.</p>
          ) : (
            <ul className="divide-y divide-stone-100">
              {filteredDrafts.map((draft) => (
                <li key={draft.id} className="flex items-center justify-between gap-3 px-4 py-3">
                  <div>
                    <p className="text-sm font-medium text-stone-900">{draft.name}</p>
                    <p className="text-xs text-stone-400">{draft.areaLabel || "Location needed"}</p>
                  </div>
                  <StatusBadge status={draft.status} />
                </li>
              ))}
              ...
              {filteredSuggestions.map((s) => (
                <li key={s.id} className="flex items-center justify-between gap-3 px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-stone-400">{s.areaLabel || "Location pattern"}</p>
                  </div>
                  <FreshnessBadge status={s.confidence} label={s.confidence} />
                </li>
              ))}
            </ul>
          )}
...
function DashboardCard({ title, value, detail, isActive, onClick }: { title: string; value: number; detail?: string; isActive: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        "cursor-pointer rounded-md border p-5 text-left shadow-sm transition",
        isActive
          ? "border-stone-950 bg-stone-950 text-stone-100 hover:bg-stone-800 active:bg-stone-700"
          : "border-stone-200 bg-white hover:border-stone-400 hover:bg-stone-50 active:bg-stone-100",
      )}
    >
      <p className={cn("text-xs font-semibold uppercase tracking-[0.14em]", isActive ? "text-stone-400" : "text-stone-500")}>{title}</p>
      <p className={cn("mt-3 text-4xl font-semibold", isActive ? "text-white" : "text-stone-900")}>{value}</p>
      {detail ? <p className={cn("mt-2 text-sm", isActive ? "text-stone-400" : "text-stone-500")}>{detail}</p> : null}
    </button>
  );
}
