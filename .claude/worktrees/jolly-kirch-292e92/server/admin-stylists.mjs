...
export function registerAdminStylistRoutes(app) {
  ...
  app.get("/api/admin/freshness-saved-checks", requireAdmin, async (_req, res) => {
    const store = await readJson(freshnessChecksPath, { meta: { updatedAt: null, checkedCount: 0, total: 0 }, checks: [] });
    res.json({
      ok: true,
      checks: store.checks || [],
