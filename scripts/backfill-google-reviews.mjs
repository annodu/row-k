...
function isAddressSourcePlatform(bookingUrl) {
...
async function fetchPreciseAddress(bookingUrl) {
  try {
...
function regionCorroborates(salon, formattedAddress) {
...
async function main() {
  ...
  for (const salon of targets) {
    try {
      ...
      if (!place) {
        results.set(salon.id, { googlePlaceId: null, googleReviewCount: 0, googleMapsUri: "", googleMatchConfidence: "no-match", googleCheckedAt: new Date().toISOString() });
      ...
      if (highConfidence) {
        highConfidenceWithReviews += place.userRatingCount > 0 ? 1 : 0;
      } else {
        lowConfidence += 1;
      }

      results.set(salon.id, {
      ...
      console.log(`${highConfidence ? "OK  " : "LOW "} ${salon.name} -> "${place.displayName?.text}" (${place.formattedAddress}) : ${place.userRatingCount || 0} reviews, nameScore=${nameScore.toFixed(2)}`);
    } catch (error) {
      failed += 1;
...
function extractPostcodeToken(address) {
